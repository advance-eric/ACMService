...
namespace ACMService.Helpers
{
    public static class EmailHelper
    {
        public static void SendNewEntryEmail(int ofiId, int ofiEntryId)
        {
            ...
            var db = Helpers.DBHelper.GetDBContext();
        ...
        {
            using (var db = Helpers.DBHelper.GetDBContext())
            {
        ...
        public static void SendEmailToUserID(string id, string subject, string emailBody)
        {
            using (var db = Helpers.DBHelper.GetDBContext())
            {
                var emailAddress = db.AspNetUsers.Where(m => m.Id == id).Select(m => m.Email).ToList();
                EmailHelper.SendEmail(emailAddress, subject, emailBody);
            }
        ...
        public static string DataTabletoHTML(DataTable dt)
        {
            ...
            myBuilder.Append("style='border: solid 1px black; font-size:10.0pt; font-family:Arial; width: 75%; border-collapse:collapse;'>");

            myBuilder.Append("<tr align='left' valign='top''>");
            foreach (DataColumn myColumn in dt.Columns)
            {
                myBuilder.Append("<td align='left' valign='top' style='border:1px solid black;'>");
                myBuilder.Append("<strong>" + myColumn.Caption + "</strong>");
                myBuilder.Append("</td>");
            }
            myBuilder.Append("</tr>");
